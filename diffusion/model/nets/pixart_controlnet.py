...
class ControlPixArtMSHalf(ControlPixArtHalf):
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, c=None, **kwargs):
        ...
        x = self.unpatchify(x)  # (N, out_channels, H, W)
        return x
from ctrlnet.diffusers.pixart_transformer_2d import PixArtTransformer2DModel

class ControlPixArtTransformer2D(ControlPixArtHalf):
    def __init__(self, base_model: PixArtTransformer2DModel, copy_blocks_num: int = 13) -> None:
        super().__init__(base_model=base_model, copy_blocks_num=copy_blocks_num)
    ...
    def forward(self, x, timestep, y, mask=None, data_info=None, c=None, **kwargs):
        ...
        x = self.base_model.pos_embed(x)
        t = self.base_model.adaln_single.emb.timestep_embedder(timestep)
        t0 = self.base_model.adaln_single.linear(t)
        y = self.base_model.caption_projection(y)

        x = auto_grad_checkpoint(self.base_model.transformer_blocks[0], x, None, y, None, t0, **kwargs)

        if c is not None:
            for index in range(1, self.copy_blocks_num + 1):
                c, c_skip = auto_grad_checkpoint(self.controlnet[index - 1], x, None, y, None, t0, c, **kwargs)
                x = auto_grad_checkpoint(self.base_model.transformer_blocks[index], x + c_skip, None, y, None, t0, **kwargs)
        
            for index in range(self.copy_blocks_num + 1, self.total_blocks_num):
                x = auto_grad_checkpoint(self.base_model.transformer_blocks[index], x, None, y, None, t0, **kwargs)
        else:
            for index in range(1, self.total_blocks_num):
                x = auto_grad_checkpoint(self.base_model.transformer_blocks[index], x, None, y, None, t0, **kwargs)

        x = self.base_model.norm_out(x)
        shift, scale = self.base_model.scale_shift_table.chunk(2, dim=1)
        x = x * (1 + scale.to(x.device)) + shift.to(x.device)
        x = self.base_model.proj_out(x)
        x = x.squeeze(1)
        x = x.reshape(shape=(-1, self.base_model.out_channels, self.base_model.height, self.base_model.width))
        return x
